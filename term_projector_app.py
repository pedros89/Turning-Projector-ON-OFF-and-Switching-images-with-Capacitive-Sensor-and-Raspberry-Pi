...
path = "Desktop/projector_app/images/"  # folder in which there are the images we want to project FILL IN THE PATH TO YOUR IMAGE FOLDER HERE
images = []
...
def cambia_immagine(nome):
    ...
    while True:
        ...
        cv2.setWindowProperty("window", cv2.WND_PROP_FULLSCREEN,1)  # it adapts the image to the screen maintaining the screen size
...
if __name__ == "__main__":
    ...
    path_f = []
    for imm in f:
        path_f.append(path + imm)
        
    print("Creating images... Please wait...")
    for imm in path_f:  # select one images one at the time
        images.append(salva_immagine(imm))  # create the images one at the time
    ...
    while True:  # infinite loop
        if touch.is_pressed:  # if the button (capacitive touch in this case) is not pressed
            ...
            if flag == 0:
                ...
                while tempo < 30000:  # time to wait between a touch command and the next
                    if boold:
                        print("one second is passed", tempo + 1)
                    tempo += 1  # increase time
                    if touch.is_pressed:  # Botton not pressed
                        if tempo >= 10000:  # if time is more than 35 change projector state
                            if boold:
                                projector = Projector.from_address("192.168.2.59")  # reopen WLAN communication with projector

                                if not projector.authenticate("panasonic"):  # reauthentication
                                    print("Authentication failed")
                                else:
                                    print("Authentication successful")
                            ...
                            flag = 1
                            break
                        if tempo < 10000:  # if time is less than 35ms so the "change image script" is started
                            if boold:
